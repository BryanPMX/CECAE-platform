    ...
    <>
      ...
      <section className="page-hero">
        <div className="section-shell max-w-4xl text-center">
          ...
          <p className="text-pretty mx-auto mt-5 max-w-3xl text-base leading-7 text-white/76 sm:text-lg sm:leading-8">
            {t('privacy.intro')}
          </p>
        </div>
      </section>
          ...
          <div className="mt-10 grid gap-5">
            {sections.map((section) => (
              <article key={section.title} className="rounded-lg border border-line bg-white p-6 shadow-sm">
                <h2 className="text-balance text-center font-display text-2xl font-bold text-navy">{section.title}</h2>
                <p className="text-pretty mt-3 text-left leading-7 text-midGray">{section.body}</p>
              </article>
            ))}
          </div>
        </div>
